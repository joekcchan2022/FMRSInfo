...
{
    [super viewDidLoad];
    self.rosterWebView.delegate = self;
    // Set Button
    ...
    NSString *urlString = [NSString stringWithFormat:@"http://m.dsd.gov.hk/mfmrs/roster/%@.pdf", link];
    
    NSURL * url = [NSURL URLWithString:[urlString stringByAddingPercentEscapesUsingEncoding:NSASCIIStringEncoding]];
    
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    [self.rosterWebView loadRequest:request];
    [self.rosterWebView setScalesPageToFit:YES];
    }
...
{   if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")) {
    ...
    NSString *filePath = [resourceDocPath stringByAppendingPathComponent:@"Roster.pdf"]; //roster
